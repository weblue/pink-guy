...
- `phase0/direct/store.mjs:51` creates the central SQLite schema and owns all
  durable task, run, audit, and orchestrator-lease state.
- `phase0/direct/store.mjs:435` expires leases, while
  `phase0/direct/store.mjs:441` through `phase0/direct/store.mjs:525` register,
  authorize, heartbeat, release, and list one active orchestrator per project.
- `phase0/direct/control-plane.mjs:627` is the central HTTP routing boundary.
  The existing `POST /api/tasks/:id/sessions` route starts a phase-scoped run
  only for an authorized project orchestrator.
- `phase0/scripts/project-orchestrator.mjs` currently registers, heartbeats,
  and releases a lease, but it cannot consume work.
- The loopback operator shell in `phase0/direct/control-plane.mjs:51` is
  read-only and does not display control-loop state.

The code remains under `phase0/` for this slice because those modules are the
tested selected foundation, not disposable throwaway code. A source-tree
graduation will be a separate mechanical change after Phase 1 establishes a
stable module boundary; combining it here would create review churn and
invalidate path-bound Phase 0 evidence.
...
Extend `phase0/scripts/project-orchestrator.mjs` with a bounded polling loop:
...
- Graduate selected runtime modules from the historical `phase0/` path once
  the Phase 1 control and PTY boundaries stabilize.
