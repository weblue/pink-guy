...
[local control-loop](../local-control-loop/TECH.md) slice
...
`src/server/store.mjs:840` is capability-scoped and intentionally remains
agent-facing.

The local shell in `src/server/control-plane.mjs` reads the board but has no
forms. `scripts/serve.mjs` binds the selected local profile to
`127.0.0.1`.
