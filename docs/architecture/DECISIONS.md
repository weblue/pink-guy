...
| D-047 | The central API owns settlement of every accepted task execution. The project daemon performs a short idempotent execution-acceptance request and observes durable state; it does not infer command success/failure from a long HTTP request. | Proposed | The Phase 1 closure run proved that client transport can fail while an API-owned Pi run continues. One settlement authority removes that split-brain path. |
| D-048 | Stop, timeout, cancellation, and uncertain-effect recovery begin with a durable mutation fence. Evidence observed after the fence is retained as late evidence but never advances task state or automatic continuation by itself. | Proposed | Prevents a failed/reset command from racing a still-authorized agent while preserving potentially useful checkpoints for audit. |
| D-049 | Only the human owner may accept or reject an identity- and provenance-verified late checkpoint. Acceptance advances the task revision atomically, invalidates stale validation/review, and requires fresh gates. | Proposed | Recovery changes authoritative source custody and can overwrite the basis for later work; it should be explicit without discarding independently provable work. |
