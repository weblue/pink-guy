...
| D-046 | Ready-queue ordering, eligibility, capacity claims, command creation, and sub-agent spawning belong to a deterministic model-less scheduler. Pi refines and proposes task graphs and may explicitly release a task, but it does not pop the runnable queue. | Proposed | The maintenance dogfood proved deterministic test/review continuation while exposing initial implementation selection as the remaining model-controlled scheduling decision. Explicit dispatch policy and stable priority/release ordering preserve intent without spending tokens on queue mechanics. |
